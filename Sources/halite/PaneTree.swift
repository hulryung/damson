...
final class PaneNode {
    enum Kind {
        case leaf(session: HaliteSession, surface: HaliteSurfaceView)
    ...
    static func leaf(_ session: HaliteSession) -> PaneNode {
    ...
    func leaves() -> [(session: HaliteSession, surface: HaliteSurfaceView)] {
        switch kind {
