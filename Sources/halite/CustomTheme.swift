...
/// 커스텀 테마의 hex 색 저장 모델. UserDefaults("halite.customTheme")에 JSON.
struct CustomThemeData: Codable, Equatable {
    ...
    static var defaultData: CustomThemeData {
        let h = HaliteTheme.defaultDark.toHexColors()
        return CustomThemeData(background: h.bg, foreground: h.fg, cursor: h.cursor, ansi: h.ansi)
    }

    func toTheme() -> HaliteTheme {
        HaliteTheme.custom(bg: background, fg: foreground, cursor: cursor, ansi: ansi)
    }
}

enum CustomTheme {
    private static let key = "halite.customTheme"
