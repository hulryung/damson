...
final class ClaudeSessionRegistryTests: XCTestCase {
    ...
    private func write(pid: pid_t = getpid(), status: String = "busy", sessionId: String = "s-1",
        ...
        // `throws` rather than `try!`: a fixture that cannot be encoded is a bug in the
        // test, and it should say so where it happened instead of trapping the whole run.
    ...
    func testReadsALiveRecord() throws {
        try write(status: "waiting", waitingFor: "permission to edit main.swift")
        let r = makeRegistry()
        r.refresh()
    ...
    func testUnknownStatusStillProducesARowAndNoBadge() throws {
        try write(status: "compacting")
        let r = makeRegistry()
        r.refresh()
    ...
    func testRecordForADeadProcessIsIgnored() throws {
        // pid 0 is never a live user process; the filename is what the registry keys on.
        try write(pid: 999_999, bodyPID: 999_999)
        let r = makeRegistry()
        r.refresh()
    ...
    func testPIDMismatchBetweenFilenameAndBodyIsRejected() throws {
        try write(pid: getpid(), bodyPID: getpid() + 1)
        let r = makeRegistry()
        r.refresh()
        XCTAssertNil(r.session(forForegroundPID: getpid()))
    }

    func testMalformedAndForeignFilesAreIgnored() throws {
        let live = getpid()
    ...
    func testInPlaceRewriteIsPickedUp() throws {
        try write(status: "busy")
        let r = makeRegistry()
        r.refresh()
    ...
    func testLookupWithNoForegroundPIDIsNil() throws {
        try write()
        let r = makeRegistry()
        r.refresh()
