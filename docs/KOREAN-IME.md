...
halite-swift는 두 층을 다 잡았다. 아래는 두 번째 race를 어떻게 발견하고 해결했는지의 기록.
...
halite Rust 문서 (`~/dev/halite/docs/KOREAN-IME.md`)가 7가지 가설을 다 검증해서 `.app`이 유일한 per-switch 해법임을 입증했다. halite-swift는 그 결론을 출발점으로 삼아 우리 환경에 적용하면서, **`.app`만으로는 per-launch race가 남는다**는 새 관찰을 얻었다.
...
raw binary로 실행되면 자기 자신을 `~/Library/Caches/halite/Halite.app` 안에 minimal bundle로 wrap한 뒤 `open -F` (fresh, saved-state 복원 안 함)로 relaunch하고 원본은 `exit(0)`.

```
~/Library/Caches/halite/Halite.app/
├── Contents/
...
### Layer 2: IME warmup (`Sources/HaliteTerminal/HaliteTerminalView.swift`)
...
- `Sources/HaliteTerminal/HaliteTerminalView.swift` — Layer 2 (`warmupIMEIfNeeded` + `isWarmingUpIME` 플래그)
